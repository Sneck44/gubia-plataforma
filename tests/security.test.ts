...
import { catalogInput, catalogs } from "../src/lib/catalogs.ts";
const promoInput = (f: FormData) => catalogInput(catalogs.codigos, f);
const campaignInput = promoInput;
...
test("consulta is read-only and unknown roles fail closed", () => {
  for (const permission of [
    ...
    assert.equal(can("consulta", permission), false);
...
test("rejects invalid discount and malformed codes without silently changing them", () => {
  for (const discount_value of ["NaN", "Infinity", "-1", "101", ""])
    assert.throws(() => promoInput(form({ discount_value })));
  for (const code of ["ab", "ABC XYZ", "ABC<script>", "A".repeat(49)])
    assert.throws(() => promoInput(form({ code })));
  assert.equal(promoInput(form({ code: "promo-26" })).code, "PROMO-26");
});
test("usage limits must be positive whole integers", () => {
  for (const usage_limit of ["0", "-1", "1.5", "1e3", "2147483648", "Infinity"])
    assert.throws(() => promoInput(form({ usage_limit })));
...
test("campaign dates must exist and close after opening in Mexico central time", () => {
  ...
  assert.throws(() =>
    campaignInput(
      form({ starts_at: "2026-09-26T12:00", ends_at: "2026-09-26T11:59" }),
    ),
  );
  assert.throws(() =>
    campaignInput(
      form({ starts_at: "2026-09-26T12:00", ends_at: "2026-09-26T12:00" }),
    ),
  );
  assert.equal(
    campaignInput(form({ starts_at: "2026-09-26T12:00" })).starts_at,
...
test("validates names, text limits and optional campaign references", () => {
  ...
  const f = form();
  f.set("name", new Blob(["test"]), "name.txt");
  assert.throws(() => promoInput(f));
});
